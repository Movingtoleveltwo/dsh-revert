...
export class ShadowGit {
  ...
  getGitDir(workspacePath) {
    ...
    // 【关键修复】确保所有现有和新建的 ShadowGit 都将 .dsh 数据库目录排除，防止被强制还原破坏 DSH 底层 JSONL 引擎的时序
    const excludePath = join(gitDir, 'info', 'exclude');
    if (existsSync(join(gitDir, 'info'))) {
      const currentExclude = existsSync(excludePath) ? require('node:fs').readFileSync(excludePath, 'utf8') : '';
      if (!currentExclude.includes('.dsh/')) {
