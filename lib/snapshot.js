...
export class ShadowGit {
  ...
  getGitDir(workspacePath) {
    ...
    if (!existsSync(gitDir)) {
      ...
      try {
        ...
        console.error('[dsh-revert] git init failed:', err.message);
      }
    }
    
    // 确保 ShadowGit 将 .dsh, .git, node_modules, dist 等非源码大文件彻底排除，保证快照秒级完成
    ...
    if (existsSync(excludeDir)) {
      ...
      if (!currentExclude.includes('node_modules')) {
        writeFileSync(excludePath, (currentExclude + '\n' + STANDARD_EXCLUDE).trim() + '\n', 'utf8');
      }
    }
    return gitDir;
  }
  ...
  takeSnapshot(workspacePath, message = 'snapshot') {
    ...
    try {
      ...
      execFileSync(
        'git',
        ['-c', 'user.name=DSH Revert', '-c', 'user.email=revert@dsh.local', '--git-dir=' + gitDir, '--work-tree=' + workspacePath, 'commit', '-m', message, '--allow-empty'],
        { cwd: workspacePath, stdio: 'pipe', timeout: 5000, encoding: 'utf8' }
      );
      const hash = execFileSync('git', ['--git-dir=' + gitDir, 'rev-parse', 'HEAD'], {
        cwd: workspacePath, stdio: 'pipe', timeout: 3000, encoding: 'utf8',
      }).trim();
      return { commitHash: hash, timestamp: Date.now() };
    } catch (err) {
      console.warn('[dsh-revert] snapshot failed:', err.message);
      return null;
    }
  }
  ...
  restoreSnapshot(workspacePath, commitHash) {
    ...
    try {
      ...
      console.error('[dsh-revert] restore failed:', err.message);
      return false;
    }
  }
}
